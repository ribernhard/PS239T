...
This and all following files draw heavily on Rochelle Terman's 2016 guest lecture from last year, which is in turn based upon her full-length course on text analysis: https://github.com/rochelleterman/text-analysis-dhbsi/. Rochelle was the creator of PS239T and does cutting-edge work in this area, so we're following her lead! 
...
```{r}
setwd("~/Desktop/PS239T/13_text-analysis-r")
rm(list=ls())
...
# Load data as corpus
challengecorpus <- Corpus(DirSource("Data/biggest_challenge/"))

# Pre-processing of text
challengecorpus <- tm_map(challengecorpus, removePunctuation) 
challengecorpus <- tm_map(challengecorpus, removeNumbers) 
challengecorpus <- tm_map(challengecorpus, content_transformer(tolower)) 
challengecorpus <- tm_map(challengecorpus, removeWords, stopwords("english"))
challengecorpus <- tm_map(challengecorpus, removeWords, c("biggest", "challenge", "campaign", "race", "run", "ran", "running", "office", "also", "difficult")) 
challengecorpus <- tm_map(challengecorpus, stripWhitespace) 

# Stemmed document - just an option 
challengecorpusstemmed <- tm_map(challengecorpus, stemDocument) 

# If needed to see text: writeLines(as.character(challengecorpus[1]))
# If needed to see text: writeLines(as.character(challengecorpusstemmed[1]))

# Create document-term matrices

# Non-stemmed
tdm <- TermDocumentMatrix(challengecorpus)   
tdm_order <- as.matrix(tdm)
tdm_order <- tibble::rownames_to_column(as.data.frame(tdm_order))
names(tdm_order)<-c("word", "frequency")
tdm_order <- tdm_order[order(-tdm_order$frequency),]

# Stemmed
tdm_stemmed <- TermDocumentMatrix(challengecorpusstemmed)   
tdm_stemmed_order <- as.matrix(tdm)
tdm_stemmed_order <- tibble::rownames_to_column(as.data.frame(tdm_order))
names(tdm_stemmed_order)<-c("word", "frequency")
tdm_stemmed_order <- tdm_order[order(-tdm_order$frequency),]

colorVec = colorRampPalette(c("#151A64", "#4292C6"))(nrow(tdm_order)) # creating automatically fading palette

# wordcloud2 is more reliable than earlier package 'wordcloud', so we'll use that
wordcloud2(tdm_order, 
          rotateRatio = 0, 
          color = colorVec,
          shape = 'circle')
    ## Note - must open plot in new window to save as .png - doesn't appear to work with
    ## R's png function 
```
...
### Exercise: Make a DTM out of your own corpus or one of the toy copora.
